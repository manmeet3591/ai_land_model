...
# -------------------- Model --------------------
# (Use your existing UNet and block classes – unchanged)
# [DoubleConv, Down, Up, OutConv, DepthwiseSeparableConv3d, UNet definitions here...]

# Paste model code here as-is
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return logits
# -------------------- Settings --------------------
year = 2018  # 👈 Control this to change the training year
...
# Filter by year and get daily mean
...
for t in range(len(ds_daily.dayofyear) - 1):
    ...
    input_healpix = torch.stack([regridder(x.to(dtype=torch.double)).reshape(12, nside, nside) for x in x_tensor])
    target_healpix = torch.stack([regridder(y.to(dtype=torch.double)).reshape(12, nside, nside) for y in y_tensor])


    input_samples.append(input_healpix)
    target_samples.append(target_healpix)

    X = torch.stack(input_samples)
    Y = torch.stack(target_samples)
    ...
    # -------------------- Model Init --------------------
    model = UNet(n_channels=X.shape[1], n_classes=Y.shape[1], out_activation=None).to(device)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        print(f"[{year}] Epoch {epoch:02d} - Loss: {total_loss:.6f}")

    # -------------------- Evaluation --------------------
    model.eval()
    with torch.no_grad():
        sample_input = X[0].unsqueeze(0).to(device)
        sample_output = model(sample_input).squeeze(0).cpu()
    print("✅ Done. Predicted shape:", sample_output.shape)  # [8, 12, 64, 64]
