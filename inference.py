...
###############################################
##########Roll over forecast ###############
# Initialize a list to store the forecasted soil variables

forecasted_soil_vars = []

# Loop over the next 10 days (starting from day 1, as day 0 has already been used for the first forecast)
for t in range(1, 11):  # Generate forecast for the next 10 days
    print(f"Generating forecast for day {t}")
    ...
    for var in vars_to_use:
        ...
        # For the first time step (t == 1), use all variables (atmospheric + soil)
        if t == 1:
            # Use the actual values for all variables
            v = ds_daily[var].isel(dayofyear=t-1)  # Use day t-1 for the first forecast (as it's indexed from 0)
            if 'latitude' in v.dims and not v.latitude.values[0] < v.latitude.values[-1]:
                v = v.sortby('latitude')
            v = v.interpolate_na(dim="latitude", method="linear", fill_value="extrapolate")
            v = v.interpolate_na(dim="longitude", method="linear", fill_value="extrapolate")
            ...
            # For subsequent forecasts, use the forecasted soil variables for previous day
            if var in ['volumetric_soil_water_layer_1', 'volumetric_soil_water_layer_2', 
                       'volumetric_soil_water_layer_3', 'volumetric_soil_water_layer_4', 
                       'soil_temperature_level_1', 'soil_temperature_level_2', 
                       'soil_temperature_level_3', 'soil_temperature_level_4']:
                # Use the forecasted values for soil variables from the previous day
                forecasted_soil_var = forecasted_soil_vars[-1][soil_vars.index(var)]
                input_vars.append(torch.tensor(forecasted_soil_var, dtype=torch.float32))
                # Regrid to Healpix
                print('forecasted_soil_var.shape = ', forecasted_soil_var.shape)
#                input_vars.append(regridder(torch.tensor(v_norm.values, dtype=torch.float64)).reshape(12, nside, nside).float())

            else:
                # For atmospheric variables, use the actual values from the dataset
                v = ds_daily[var].isel(dayofyear=t-1)  # Use actual values from the dataset for the current day
                if 'latitude' in v.dims and not v.latitude.values[0] < v.latitude.values[-1]:
                    v = v.sortby('latitude')
                v = v.interpolate_na(dim="latitude", method="linear", fill_value="extrapolate")
                v = v.interpolate_na(dim="longitude", method="linear", fill_value="extrapolate")
                v_norm = (v - norm_stats[var]['min']) / (norm_stats[var]['max'] - norm_stats[var]['min']).unsqueeze(0)
#                input_vars.append(torch.tensor(v_norm.values, dtype=torch.float32))
                # Regrid to Healpix
                print('v_norm.shape = ', v_norm.shape)
                print('healpix v_norm.shape', regridder(torch.tensor(v_norm.values, dtype=torch.float64)).reshape(12, nside, nside).float().shape)
                input_vars.append(regridder(torch.tensor(v_norm.values, dtype=torch.float64)).reshape(12, nside, nside).float())
    ...
    # Store the forecast for this time step
    forecast_np = forecast.squeeze(0).cpu().numpy()
    forecasted_soil_vars.append(forecast_np)
